...
int main(int argc, char *argv[])
{
	...
	cookie = 0;
	msg_send(conn_b, NULL, 0xc0000000 | cookie, 0, 0, 0, KDBUS_DST_ID_BROADCAST, 0, 0);
	...
	for (count = 0;; count++) {
		...
		if (fds[0].revents & POLLIN) {
			...
			msg_recv(conn_a);
			msg_send(conn_a, NULL, 0xc0000000 | cookie++, 0, 0, 0, conn_b->id, 0, 0);
		}

		if (fds[1].revents & POLLIN) {
			msg_recv(conn_b);
			msg_send(conn_b, NULL, 0xc0000000 | cookie++, 0, 0, 0, conn_a->id, 0, 0);
		}
